...
pub trait ChannelResolving {
    fn get_endpoints_async(
        &self,
        id: u64,
    ) -> Pin<Box<dyn Future<Output = Result<EndpointData, Canceled>> + Send>>;
    fn is_endpoint_cached(&self, id: u64) -> bool;
    fn get_node(&self, node_id: NodeId) -> Option<lightning::ln::msgs::UnsignedNodeAnnouncement>;
}

pub struct CachingChannelResolving<L: Deref + Send + std::marker::Sync + 'static>
where
    L::Target: Logger,
{
    logger: L,
...
impl<L: Deref + Send + std::marker::Sync + 'static> CachingChannelResolving<L>
where
    L::Target: Logger,
{
...
impl<L: Deref + Send + std::marker::Sync + 'static> ChannelResolving for CachingChannelResolving<L>
where
    L::Target: Logger,
{
    fn get_endpoints_async(
...
impl<L: Deref + Send + std::marker::Sync + 'static> MessageSendEventsProvider
    for CachingChannelResolving<L>
where
    L::Target: Logger,
{
...
impl<L: Deref + Send + std::marker::Sync + 'static> RoutingMessageHandler
    for CachingChannelResolving<L>
where
    L::Target: Logger,
{
    fn handle_node_announcement(
        ...
        self.node_cache
            .lock()
            .unwrap()
            .insert(msg.contents.node_id, msg.contents.clone());
        Ok(false)
    }
    ...
    fn handle_channel_announcement(
        ...
        self.chan_id_cache.lock().unwrap().insert(
